...
call extend(g:vimrc_altercmd_dic, #{
  ...
  \})
nnoremap <silent> <Space>gl <Cmd>call ddu#start(#{name: 'git_log', sources: [#{name: 'git_log'}]})<CR>
nnoremap <silent> <Space>gbr <Cmd>call ddu#start(#{sources: [#{name: 'git_branch'}]})<CR>
...
function s:gitDiffTree(args) abort
  call ddu#start(#{sources: [#{name: 'git_diff_tree', params: #{commitHash: a:args.items[0].action.hash}}]})
endfunction
call ddu#custom#action('source', 'git_log', 'diffTree', function('s:gitDiffTree'))
