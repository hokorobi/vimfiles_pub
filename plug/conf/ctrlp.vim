...
nnoremap <Space>fo <Cmd>call CtrlPDefaultInput('CtrlPLine', '{{{ ')<CR>
nnoremap <Space>*  <Cmd>call CtrlPDefaultInput('CtrlPLine', $'{expand("<cword>")} ')<CR>
xnoremap <Space>*  <Cmd>call CtrlPDefaultInput('CtrlPLine', $'{lazy#GetCurrentSelection()} ')<CR>
nnoremap <Space>u  <Cmd>CtrlPMRUFiles<CR>
nnoremap <Space>ff <Cmd>CtrlP<CR>
nnoremap <Space>fv <Cmd>CtrlP ~/vimfiles<CR>
" nnoremap <Space>fq <Cmd>CtrlPQuickfix<CR>
" nnoremap <Space>fr <Cmd>call CtrlpRepository()<CR>
xnoremap <Space>fr <Cmd>call CtrlpRepository()<CR>
...
" }}}
" hook_source {{{
function! CtrlpRepository() abort
  " FIXME リポジトリ名を取得できていないみたい。
  let repo = lazy#GetRepositoryName()
  if repo ==# ''
    execute 'CtrlP ~/_vim/dein/repos/github.com'
    return
  endif

  let isRepo = input(printf('CtrlP %s ? [Y/n]', repo))
  if isRepo ==# '' || isRepo ==# 'y' || isRepo ==# 'Y'
    execute $'CtrlP ~/_vim/dein/repos/github.com/{repo}'
  endif
endfunction

" CtrlP のコマンドと初期入力値を指定して実行
function! CtrlPDefaultInput(cmd, input)
  try
...
" }}}
" rst {{{
nnoremap <buffer> <Space>fo <Cmd>call CtrlPDefaultInput('CtrlPLine', 'list-table ')<CR>
" }}}
" go {{{
...
" }}}
" toml {{{
nnoremap <buffer> <Space>fg <Cmd>call CtrlPDefaultInput('CtrlPLine', 'repo= ')<CR>
" }}}
